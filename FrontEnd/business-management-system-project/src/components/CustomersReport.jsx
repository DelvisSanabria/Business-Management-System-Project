...
import ExportCustomersReport from "./ExportToExcelBtn/ExportCustomersReport";
...
export default function VendorsReports () {
  ...
  useEffect(() => {
    const fetchData = async () => {
      axios
        .get(`http://localhost:3001/reports/salesPerClients?month=${selectedMonth}&year=${selectedYear}`, {
          params: {
            page: currentPage,
            limit: 6,
          },
        ...
        <div className="border border-[#eaecf0] m-5 p-4 mr-7 rounded-[15px] md:w-[67vw] lg:hidden">
          ...
          <div className="mt-10">
            <table className="w-[80vw] border-b-2 md:w-[63vw]">
              ...
              <tbody>
                {CustomersReport.docs &&
                  CustomersReport.docs.map((client) => (
                    <tr key={client._id} className="text-[#637381] text-center">
                      <td className="border-b p-2 px-5">
                        <div className="flex items-center w-[35px] m-5 h-[35px] bg-[#d9d9d9] rounded-[44px] shadow-[0px_4px_4px_#00000040]">
                          <img
                            src={client.avatar}
                            alt={`${client._id}-image`}
                            className="w-[30px] h-[30px] rounded-[44px]"
                          />
                        </div>
                      </td>
                      <td className="border-b p-2">{client._id}</td>
          ...
          <div className="grid grid-cols-2 mt-5">
            <div className="w-[100px] h-[31px] py-3 font-semibold text-[#667085] text-[15px] text-center tracking-[0] leading-[15px]">
              <span>Página {CustomersReport ? CustomersReport.page : ""}</span>
            </div>
            <div className="grid grid-cols-[100px_100px] mx-5">
              <button
                onClick={() => {
                  if (
                    CustomersReport &&
                    CustomersReport.hasPrevPage &&
                    CustomersReport.page > 1
                  ) {
                    handlePrevPage();
                  }
                }}
              ...
              <button
                onClick={() => {
                  if (CustomersReport && CustomersReport.hasNextPage) {
                    handleNextPage();
                  }
                }}
                className="inline-flex items-start rounded-[8px] all-[unset] box-border"
        ...
        <div className="hidden lg:block">
          <div className="border border-[#eaecf0] m-5 p-4 mr-7 w-[70vw] rounded-[15px]">
            ...
            <div className="grid grid-cols-[2fr_1fr] justify-around mt-5">
              <div className="w-[100px] h-[31px] py-3 font-semibold text-[#667085] text-[15px] text-center tracking-[0] leading-[15px]">
                <span>
                  Página {CustomersReport ? CustomersReport.page : ""}
                </span>
              </div>
              <div className="grid grid-cols-[100px_100px] mx-5">
                <button
