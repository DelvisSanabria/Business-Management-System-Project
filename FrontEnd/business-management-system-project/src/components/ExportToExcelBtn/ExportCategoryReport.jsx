...
const ExportCategoryReport = ({ ReportData,month,year,page }) => {
  const [loading, setLoading] = useState(false);

  const titulo = [{ A: "Reporte Mensual de Ventas por Categoria" }, {}];

  const longitudes = [35, 25];

  const handleDownload = () => {
    setLoading(true);

    let tabla = [
      { A: "Categoria", B: "Total Ventas" },
    ];

    ReportData.forEach((categoryData) => {
      tabla.push({
        A: categoryData.category,
        B: categoryData.totalSales,
      });
    });
  ...
  const creandoArchivo = (dataFinal) => {
    ...
    XLSX.writeFile(libro, `${month}-${year}-Category-MonthReport-Page=${page}.xlsx`);
  };
